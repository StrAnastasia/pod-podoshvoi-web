      ...
      <div className='gig-card' >
        ...
        <div className='cards-conteiner'>
          ...
          <div className="d-flex  align-items-center flex-column">
            ...
            <h1 className='gig-card-title' >{gig?.name}</h1></div>


          <Link className='card-adress' to={`/place/${gig?.adress}`}>
            <h5 onClick={findplaceHandler} data-value={gig?.adress}>{gig?.adress}</h5>
          </Link>
          ...
          {gig?.bands ? gig?.bands.map(band => {
            return <Link className='card-adress' to={`/band/${band}`}>
              <div className='' onClick={findHandler} data-value={band}>{band}</div>
            </Link>
          })
            : null}
